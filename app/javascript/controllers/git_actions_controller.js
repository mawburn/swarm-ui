...
export default class extends Controller {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        this.showNotification(
          `Successfully pulled ${data.commits_pulled} commit${data.commits_pulled === 1 ? '' : 's'} for ${instanceName}`,
          'success'
        );
        ...
        if (data.has_conflicts) {
          ...
          this.showNotification(
            'Please resolve conflicts using your preferred git tool',
            'warning'
          );
        }
      }
  ...
  async push(event) {
    ...
    try {
      ...
      if (response.ok && data.success) {
        // Show success message with animation
        this.showNotification(
          `Successfully pushed ${data.commits_pushed} commit${data.commits_pushed === 1 ? '' : 's'} for ${instanceName}`,
          'success'
        );
  ...
  async stage(event) {
    ...
    try {
      ...
      if (response.ok && data.success) {
        // Show success message with animation
        this.showNotification(
          `Successfully staged ${data.files_staged} file${data.files_staged === 1 ? '' : 's'} for ${instanceName}`,
          'success'
        );
  ...
  async reset(event) {
    ...
    if (
      !confirm(
        `Are you sure you want to reset all changes in ${instanceName}?\n\nThis will:\n• Discard ALL staged and unstaged changes\n• Remove ALL untracked files\n• Reset to the last commit\n\nThis action cannot be undone!`
      )
    ) {
      return;
    }
  ...
  async openTerminal(event) {
    ...
    if (terminalTabsElement) {
      ...
      terminalTabsElement.dispatchEvent(
        new CustomEvent('terminal:create', {
