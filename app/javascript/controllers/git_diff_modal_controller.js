...
export default class extends Controller {
  ...
  async open(event) {
    ...
    if (!sessionId || !directory) {
      console.error('Missing required data for git diff modal', {
        sessionId,
        directory,
        instanceName,
      });
  ...
  async showFile(index) {
    ...
    try {
      ...
      try {
        ...
        originalModel = this.monaco.editor.createModel(fileData.original_content || '', language);

        modifiedModel = this.monaco.editor.createModel(fileData.modified_content || '', language);
      } catch (error) {
        ...
        originalModel = this.monaco.editor.createModel(fileData.original_content || '', 'text');

        modifiedModel = this.monaco.editor.createModel(fileData.modified_content || '', 'text');
      }
  ...
  closeOnClickOutside(event) {
    ...
    const isCommentElement =
      event.target.closest('.comment-widget-container') ||
  ...
  getStatusBadge(status) {
    const badges = {
      staged:
        '<span class="text-xs px-1.5 py-0.5 bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 rounded">staged</span>',
      modified:
        '<span class="text-xs px-1.5 py-0.5 bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200 rounded">modified</span>',
      'modified+staged':
        '<span class="text-xs px-1.5 py-0.5 bg-orange-100 dark:bg-orange-900 text-orange-800 dark:text-orange-200 rounded">modified+staged</span>',
      untracked:
        '<span class="text-xs px-1.5 py-0.5 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 rounded">untracked</span>',
    };
  ...
  resetActionButtons() {
    ...
    const requestChangesButton = this.modal.querySelector(
    ...
    if (requestChangesButton) {
      ...
      requestChangesButton.className =
        'px-3 py-1.5 text-xs font-medium rounded bg-gray-300 dark:bg-gray-600 text-gray-500 dark:text-gray-400 cursor-not-allowed opacity-50';
  ...
  setupCommentGutterClick(editor, side, file) {
    ...
    editor.onMouseDown((e) => {
      ...
      if (
        ...
        const existingWidget = this.commentWidgets.find(
          (w) => w.lineNumber === lineNumber && w.side === side && !w.isDisposed
        );
  ...
  deleteComment(comment, file) {
    ...
    const hasMoreComments = fileComments.some(
      (c) => c.lineNumber === comment.lineNumber && c.side === comment.side
    );
  ...
  removeCommentDecoration(lineNumber, side) {
    ...
    const decorationIndex = this.commentDecorations.findIndex(
      (d) => d.lineNumber === lineNumber && d.side === side && d.editor === editor
    );
  ...
  updateRequestChangesButton() {
    ...
    if (totalComments === 0) {
      ...
      requestChangesButton.className =
        'px-3 py-1.5 text-xs font-medium rounded bg-yellow-600 hover:bg-yellow-700 text-white transition-colors cursor-pointer';
    }
  }
}
