...
export default class extends Controller {
  ...
  connect() {
    ...
    if (
      this.hasProjectPathInputTarget &&
      this.projectPathInputTarget.value &&
      this.hasConfigSelectTarget
    ) {
  ...
  open() {
    ...
    this.modalTarget
      .querySelector('[data-action="click->filesystem-browser#close"]')
      .classList.add('opacity-100');
    this.modalTarget
      .querySelector('.relative.transform')
      .classList.add('opacity-100', 'translate-y-0', 'scale-100');
    this.modalTarget
      .querySelector('.relative.transform')
      .classList.remove('opacity-0', 'translate-y-4', 'scale-95');
    this.navigate('~');
  }
  ...
  close() {
    ...
    this.modalTarget
      .querySelector('.relative.transform')
      .classList.add('opacity-0', 'translate-y-4', 'scale-95');
  ...
  renderFileList(entries) {
    ...
    this.fileListTarget.innerHTML = entries
      .map(
        (entry) => `
      <button type="button"
  ...
  async scanForSwarmConfigs() {
    ...
    try {
      this.configSelectTarget.innerHTML =
        '<option value="">Scanning for swarm configurations...</option>';

      const response = await fetch(
        `/filesystem/scan_swarm_configs?path=${encodeURIComponent(this.currentPathValue)}`
      );
      const data = await response.json();
      ...
      if (data.configs.length > 0) {
        ...
        const currentValue =
          this.configSelectTarget.dataset.currentValue || this.configSelectTarget.value;
        if (currentValue) {
        ...
        this.configSelectTarget.innerHTML =
          '<option value="">No swarm configuration files found</option>';
      ...
      this.configSelectTarget.innerHTML =
        '<option value="">Error scanning for configurations</option>';
    }
  }
  ...
  async projectChanged(event) {
    ...
    try {
      ...
      this.configSelectTarget.classList.remove('opacity-50', 'cursor-not-allowed');
      this.configSelectTarget.innerHTML =
        '<option value="">Loading project information...</option>';
      ...
      if (project && project.path) {
        ...
        if (defaultConfigPath) {
          ...
          const defaultOption = options.find(
            (opt) => opt.value === defaultConfigPath || opt.value.endsWith(`/${defaultConfigPath}`)
          );
